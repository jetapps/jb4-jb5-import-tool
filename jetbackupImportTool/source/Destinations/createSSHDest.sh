...
JB4ARGS=( "name" "dr" "hidden" "path" "disklimit" "rsyncbwlimit" "host" "port" "username" "timeout" "rsyncpreferip")
SSHKEY=/usr/local/jetapps/etc/jetbackup/privatekeys/root.rsa
...
function createSSHDestination () {
  ...
  echo "privatekey: $JB5SSHKEY" >> $workspace
  echo "" >> $workspace
...
########## building command ###############
CMD="jetbackup5api -F manageDestination -D 'action=create&type=SSH&name=${ARGS[name]}&disabled=1&dr=${ARGS[dr]}&hidden=${ARGS[hidden]}&options[path]=${ARGS[path]}&disk_limit=${ARGS[disklimit]}&options[rsyncbwlimit]=${ARGS[rsyncbwlimit]}&options[host]=${ARGS[host]}&options[port]=${ARGS[port]}&options[privatekey]=${JB5SSHKEY}&options[username]=${ARGS[username]}'"
#eval $CMD
echo $CMD >> $workspace
}
