...
do
  ...
  #readarray -t JB5Exists <<< "$(jetbackup5api -F listDestinations | grep -w "name: ${NAME}" | sed 's/.*name: //')"

  JB5Exists=$(jetbackup5api -F listDestinations -D "filter=$NAME" | grep -w "name: $NAME\$")
